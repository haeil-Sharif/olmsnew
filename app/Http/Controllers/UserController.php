...
class UserController extends Controller
{
    ...
    public function UserProfile_Update(Request $request)
    {
        ...
        $Letvalidator = Validator::make($request->all(), [
            'firstname' => 'regex:/^[A-Za-z ]+$/',
            'lastname' => 'regex:/^[A-Za-z ]+$/',
            'middlename' => 'regex:/^[A-Za-z ]+$/',
        ]);

        $zipvalidator = Validator::make($request->all(), [
            'zip_code' => 'regex:/^[A-Za-z ]+$/',
            'contact_no' => 'regex:/^[A-Za-z ]+$/',
        ]);
        $addvalidator = Validator::make($request->all(), [
            'address' => 'regex:/^[A-Za-z0-9,. -]+$/',
            'barangay' => 'regex:/^[A-Za-z0-9,. -]+$/',
        ]);  
        $cityvalidator = Validator::make($request->all(), [
            'city' => 'regex:/^[A-Za-z -]+$/',
        ]);
        ...
        elseif($Letvalidator->fails()){
            return response()->json(['status' => 'error' , 'message' => "Name field only accept Alphabet characters"]);
        }

        elseif($addvalidator->fails()){
            return response()->json(['status' => 'error' , 'message' => "Adress/Barangay doesnt accept Special characters such as @,#,! "]);
        }

        elseif($zipvalidator->fails()){
            return response()->json(['status' => 'error' , 'message' => "Contanct No./Zip Code only accept numbers"]);
        }

        elseif($cityvalidator->fails()){
            return response()->json(['status' => 'error' , 'message' => "City only accept Alphabet characters and '-' as valid input"]);
        }
        else{
