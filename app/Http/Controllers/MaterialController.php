...
class MaterialController extends Controller
{
    ...
    public function generateReport()
    {
        ...
        $MatDet = db::table('materials')
        ->select(DB::raw('count(materials_copies.material_copy_id) as copies'),'isbn', 'title', 'type', 'created_at',)
        ->join('materials_copies','materials_copies.materials_id', '=' ,'materials.materials_id')
        ->groupBy('materials_copies.materials_id')
        ->get();
        
        
        $MatContent = json_decode(json_encode($MatDet), true);
    
        $data = $_POST;
        // echo"<pre>";
        // die(print_r($data)); 
        $data['Material_list'] = array();
        foreach($MatContent as $materials){   
        $convert_create = date('Y-m-d', strtotime($materials['created_at'])); 
            if($convert_create >= $data['start'] && $convert_create <= $data['end']){
                $matDetails['isbn'] = ucwords(strtoupper($materials['isbn']));
                $matDetails['title'] = $materials['title'];
                $matDetails['type'] = $materials['type'];
                $matDetails['copies'] = $materials['copies'];
                $created_at = date_format(date_create($materials['created_at']), 'F d, Y');
                $matDetails['created_at'] = $created_at;
                array_push($data['Material_list'], $matDetails);
            }
        }  
        ...
        $pdf->AddPage();
        $html = view('Materials/generateReport', $data);
